...
############## ndm_parsing_scores function ######

# This function extracts the minimum and maximum EI scores per consensus area.
# It takes as input the file .acf for each consensus area. This files can be
# obtained by saving the file for GlobalMapper in VNDM-NDM.

# Arguments:
# 1) x: a list of the complete file paths to the .acf files. 

# Output: a data frame with columns Spatial_Scale, Consensus_Are, 
# Cons_Area_Min_Score, and Cons_Area_Max_Score.

ndm_parsing_cascores <- function(x) {
  
  vec_spatial_scale <- vector(mode = "numeric", length = length(x))
  vec_consarea_name <- vector(mode = "character", length = length(x))
  vec_caminie_name <- vector(mode = "numeric", length = length(x))
  vec_camaxie_name <- vector(mode = "numeric", length = length(x))
  vec_proximate_geo <- vector(mode = "character", length = length(x))
  
  for (i in seq_along(x)) {
    
    a <- readLines(x[i])
    
    index_ie <- grep(pattern = "DESCRIPTION.*", a)
    
    b <-  unique(a[index_ie]) 
    
    c <- gsub(pattern = "DESCRIPTION=SCORE_(.*)", replacement = "\\1", x = b)
    
    d <- as.numeric(do.call("c", strsplit(c, split = "-")))
    
    vec_caminie_name[i] <- range(d)[1]
    
    vec_camaxie_name[i] <- range(d)[2]
    
    vec_spatial_scale[i] <- gsub(
      pattern = "./data/([1-3])dgD_CA([0-9]+).*.acf", 
      replacement = "\\1", x[i])
    
    vec_consarea_name[i] <- paste("CA", 
                                  gsub(pattern = "./data/([1-3])dgD_CA([0-9]+)_([a-zA-Z]+).*.acf",
                                       replacement = "\\2", x[i]), 
                                  sep = " ")
    
    vec_proximate_geo[i] <- gsub(pattern = "./data/([1-3])dgD_CA([0-9]+)_([a-zA-Z]+).*.acf",
                                 replacement = "\\3", x[i])
    
  }
  
  CA_scores_df <- data.frame(
    Spatial_Scale = as.numeric(vec_spatial_scale),
    Consensus_Area = vec_consarea_name,
    Cons_Area_Min_Score = as.numeric(vec_caminie_name),
    Cons_Area_Max_Score = as.numeric(vec_camaxie_name),
    Approx_geo = as.character(vec_proximate_geo)
  )
...
# Function to rasterize the areas of endemism. Use with each spatial scale at a 
# time. Get areas to rasterize using the function ndm_list_grid.files()
